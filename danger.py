...
def method():
    ...
    if lp.isnumeric():
        pp=[]
        clear()
        ex = 'firstlast first123 57273200 last123'
        print(f'{oo("+")}{ex} (ETC)')
        lin3()
    ...
    print(f'\x1b[1;97m[✓]  Use flight mode before use ;)')
    lin3()
    def start(user):
     try:
        global loop,idx,cll
        ...
        sys.stdout.write(f'\r {R}[{W}DANGER{R}] {P}({Y}{loop}{W} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
              heads=None
              ...
              if 6==random.randint(1,300):
                 oku.append(acc)
                 print('\033[1;32m[DANGER-OK] \033[1;32m'+acc+' \033[1;32m|\033[1;32m '+pswd)
                 open('/sdcard/DANGER-Ok.txt','a').write(f'{acc}|{pswd}\n')
                 break
              else:
                   continue   
